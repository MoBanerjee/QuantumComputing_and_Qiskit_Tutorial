...
## Parameterized quantum circuits (contd.)
### Notes-:
1. Expressibility (coverage of the Hilbert space by circuit output states) is measured by the extent to which the outputted states deviate from a uniform distribution.
2. Higher the expressibility, the more the number of states that can be generated by the circuit.
3. On the other hand, entangling capability is computed using the Meyer-Wallach measure. The metric’s value is 0 for an unentangled state and 1 for a highly entangled state like the Bell state.
4. Experiments on various datasets and circuits have indicated that there is a strong correlation between classification accuracy and expressibility but a weak correlation between accuracy and entangling capability.
